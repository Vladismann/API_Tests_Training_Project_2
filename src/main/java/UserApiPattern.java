...
public class UserApiPattern extends StellarBurgerRestPattern {
    ...
    private static final String CHANGE_USER_DATA_PATH = "api/auth/login/";
    ...
    public ValidatableResponse changeDataWithAuth(UserDataPattern user) {

        String bearerToken = given().spec(getBaseSpec()).body(user).post(LOGIN_PATH).then().extract().path("accessToken");

        return given().header("Authorization", bearerToken)
                .spec(getBaseSpec()).body(user).post(CHANGE_USER_DATA_PATH).then();
    }

    @Step("Change user data without authorization")
    public ValidatableResponse changeDataWithoutAuth(UserDataPattern user) {

        return given().spec(getBaseSpec()).body(user).post(CHANGE_USER_DATA_PATH).then();
    }
